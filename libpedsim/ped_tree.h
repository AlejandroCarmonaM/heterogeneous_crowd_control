...
class LIBEXPORT Ttree {
 ...
 public:
  Ttree(Ped::Ttree* root, std::map<const Ped::Tagent*, Ped::Ttree*>* treehash, int depth,
        int maxDepth, int x, int y, int w, int h);
  virtual ~Ttree();

  // Removes all agents from this tree
  virtual void clear();

  // Adds an agent to the tree. Searches the right node and adds the agent
  // there. If there are too many agents at that node allready, a new child is
  // created.
  virtual void addAgent(const Ped::Tagent* a);

  // Updates the tree structure if an agent moves. Removes the agent and places
  // it again, if outside boundary. If an this happens, this is O(log n), but
  // O(1) otherwise.
  ...
  // within a given radius r (x+-r/y+-r is inside)
  ...
  // \param   pr The radius
  virtual bool intersects(int px, int py, int pr) const;
 ...
 protected:
  ...
  Ttree* getChildByPosition(int x, int y);
